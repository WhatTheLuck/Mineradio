...
assert.equal(context.MineradioEmoMusic.defaults.mode, 'particles', 'the recovered live face mode is the software default');
assert.equal(context.MineradioEmoMusic.defaults.meshDensity, 5, 'the recovered mesh density is the software default');
...
assert.equal(context.MineradioEmoMusic.defaults.lasers, true, 'the recovered laser state is the software default');
assert.equal(context.MineradioEmoMusic.defaults.peakSensitivity, .5, 'the recovered RMS peak sensitivity is the software default');
assert.equal(context.MineradioEmoMusic.defaults.laserDirection, 'lyrics', 'the recovered lyric-locked laser direction is the software default');
assert.equal(context.MineradioEmoMusic.defaults.gazeOffsetX, -.08, 'the recovered horizontal gaze offset is the software default');
assert.equal(context.MineradioEmoMusic.defaults.gazeOffsetY, .14, 'the recovered vertical gaze offset is the software default');
assert.equal(context.MineradioEmoMusic.defaults.pollSeconds, 10, 'VLM emotion detection defaults to a ten-second interval');
assert.equal(context.MineradioEmoMusic.defaults.telemetryMode, 'vlm', 'VLM emotion analysis is the recovered software default');
...
assert.match(source, /mode:\s*'particles'/);
...
assert.match(source, /meshDensity:\s*5/);
