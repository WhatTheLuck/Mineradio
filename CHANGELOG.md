...
- 本地歌曲首次播放时自动采用 MR 节奏分析，不再弹出模式选择窗口。
- EmoMusic 增加摄像头切换，改进人脸识别启动失败与运行异常后的恢复提示和重试。
- MuQ 文本编码遇到临时 `TextEncodeInput` 异常时自动重试一次。
...
- 新增 CyberRibbon、SpaceVenom 与 EmoMusic 视觉预设，并补充可保存的实时参数控制。
- 新增基于人脸、视线与音乐能量的 EmoMusic 表情视觉；无人脸时停止情绪生成并清除过期状态。
- 将 LLM/VLM 请求收敛到 Electron 主进程的安全边界，凭据不进入渲染器或源码。
- 优化视觉默认值、歌词行距、随机情绪向量与磁流体/星空的音频响应。
