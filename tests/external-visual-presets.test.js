...
const softwareDefaults=require('../public/vendor/source-visuals/software-defaults.json');
...
test('external visual controls omit preview buttons and auto-load each latest saved preset',()=>{
 ...
 assert.match(moduleSource,/software-defaults\.json/);
 assert.equal(softwareDefaults.space.visual.sphereCount,41);
 assert.equal(softwareDefaults.space.visual.baseSpeed,1.27);
 assert.equal(softwareDefaults.space.visual.cameraDistance,6.54);
 assert.equal(softwareDefaults.space.starfield.speedMin,6.65);
 assert.equal(softwareDefaults.space.starfield.speedMax,25.9);
 assert.match(moduleSource,/values\.space\.starfield=Object\.assign\(\{\},starfieldTriggerDefaults/);
